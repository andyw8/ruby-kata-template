...
module Minitest
  module Reporters
    ...
    class AndyReporter < BaseReporter
      include ANSI::Code
      ...
      def initialize(options = {})
        ...
        @show_assertions = options.fetch(:show_assertions, true)
        @options = options
      end
      ...
      def suite_result
        if failures > 0 then :fail
        elsif errors > 0 then :error
        elsif skips > 0 then :skip
        else
          :pass
        end
      end
      ...
      def location(exception)
        ...
        exception.backtrace.reverse_each do |s|
          break if /in .(assert|refute|flunk|pass|fail|raise|must|wont)/.match?(s)
      ...
      def result_line
        result = ["#{count} tests"]
        result << "#{assertions} assertions" if @show_assertions
        result << "#{failures} failures"
        result << "#{errors} errors" if errors > 0
        result << "#{skips} skips" if skips > 0
        result.join(", ")
      end
...
Minitest::Reporters.use!([Minitest::Reporters::AndyReporter.new(color: true, status_line: false, run_options: false, show_assertions: false)])
